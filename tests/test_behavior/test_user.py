...
class TestAccountBlueprint(BaseTestCase):
    ...
    def test_user_login_behaves_correctly(self):
        ...
        with self.client:
            with self.init_all_user_tables_with_auth_user():
                self.app.config['IN_FAKE_TEST'] = True

                response = self.client.post(
                    'account/login',
                    data=dict(email="tester@test.com", password="testing"),
                    follow_redirects=True
                )
                self.assertEqual(response.status_code, 200)
                self.assertIn(u'登录成功', response.data.decode('utf8'))
                self.assertTrue(current_user.email == "tester@test.com")
                self.assertTrue(current_user.username == "tester")
                self.assertTrue(current_user.is_active())
                self.assertTrue(current_user.is_authenticated())
                self.assertFalse(current_user.is_anonymous())
                self.assertEqual(int(current_user.get_id()), 2)
