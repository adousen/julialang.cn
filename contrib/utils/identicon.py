...
class DonRenderer(IdenticonRendererBase):
    ...
    def decode(self, code):
        # decode the code, less than 32 bytes(2147483648)
        # img with color should be more than 2**16=65536
...
def generate_avatar(id, size=32):
    """Generate a avatar image by user's id
    ...
    int_id = int(str_id)
    if int_id > 10:
        int_id = int(str_id[len(str_id) - 1])
    rgbs = [(8, 112, 152), (72, 200, 104), (216, 56, 56),
            (48, 168, 232), (144, 240, 32), (248, 184, 16),
            (88, 216, 224), (240, 120, 24), (208, 56, 104), (184, 64, 200)]

    rgb_color = rgbs[int_id-1]
    red = rgb_color[0] >> 3
    green = rgb_color[1] >> 3
    blue = rgb_color[2] >> 3

    color = red << 26 | green << 21 | blue << 16

    avatar_code = 6505*int_id + int(str_id)
    avatar_code |= color

    img = render_identicon(avatar_code, size)
